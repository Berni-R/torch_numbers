...
class Encoder(nn.Module):
    ...
    def __init__(
            ...
            out_dim: int = 256,
            layer_dims: tuple[int, int, int, int] = (12, 32, 256, 256),
    ):
        super().__init__()
        ...
        self.model = nn.Sequential(
            ...
            nn.Conv2d(layer_dims[2], layer_dims[3], (2, 2), groups=8),
            nn.LeakyReLU(relu_negative_slope, inplace=True),
            nn.Flatten(),
            nn.Linear(layer_dims[3], out_dim),
        )
...
class Decoder(nn.Module):
    ...
    def __init__(
            ...
            norm: Literal['batch', 'instance'] = 'instance',
            in_dim: int = 256,
            layer_dims: tuple[int, ...] = (256, 64, 32, 12),
    ):
        super().__init__()
        ...
        self.model = nn.Sequential(
            ...
            nn.ConvTranspose2d(layer_dims[0], layer_dims[1], kernel_size=(2, 2)),
            nn.LeakyReLU(relu_negative_slope, inplace=True),
            {'batch': nn.BatchNorm2d(layer_dims[1]), 'instance': nn.InstanceNorm2d(layer_dims[1])}[norm],
            nn.ConvTranspose2d(layer_dims[1], layer_dims[2], kernel_size=(4, 4), stride=(2, 2)),
            nn.LeakyReLU(relu_negative_slope, inplace=True),
            {'batch': nn.BatchNorm2d(layer_dims[2]), 'instance': nn.InstanceNorm2d(layer_dims[2])}[norm],
            nn.ConvTranspose2d(layer_dims[2], layer_dims[3], kernel_size=(4, 4), stride=(2, 2)),
            nn.LeakyReLU(relu_negative_slope, inplace=True),
            {'batch': nn.BatchNorm2d(layer_dims[3]), 'instance': nn.InstanceNorm2d(layer_dims[3])}[norm],
            nn.ConvTranspose2d(layer_dims[3], 1, kernel_size=(2, 2), stride=(2, 2)),
        )
